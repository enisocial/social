-- Fix get_online_friends_optimized to return correct unread_count for current user
...
BEGIN
  ...
  friend_conversations AS (
    SELECT DISTINCT ON (cp1.user_id)
    ...
    ORDER BY cp1.user_id, cp2.unread_count DESC
  )
  SELECT 
    uf.friend_user_id,
    p.name,
