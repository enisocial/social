-- Fix get_smart_feed_optimized to properly cast enum types to text
DROP FUNCTION IF EXISTS get_smart_feed_optimized(uuid, text, integer, integer);
...
BEGIN
  ...
  post_stats AS (
    SELECT 
      ...
      p.content, 
      p.media_url, 
      p.media_type::TEXT,  -- Cast enum to text
      p.privacy::TEXT,     -- Cast enum to text
      p.created_at, 
      p.updated_at, 
      p.user_id,
      prof.username, 
