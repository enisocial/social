-- Fix security warnings for existing functions by adding search_path
CREATE OR REPLACE FUNCTION public.get_personalized_feed(user_id_param uuid, filter_type text DEFAULT 'all'::text, limit_param integer DEFAULT 20, offset_param integer DEFAULT 0)
 RETURNS TABLE(id uuid, content text, media_url text, media_type media_type, privacy post_privacy, created_at timestamp with time zone, updated_at timestamp with time zone, user_id uuid, username text, name text, avatar_url text, likes_count bigint, comments_count bigint, user_liked boolean, relevance_score numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ORDER BY 
    CASE 
      WHEN filter_type = 'recent' THEN EXTRACT(EPOCH FROM p.created_at)
