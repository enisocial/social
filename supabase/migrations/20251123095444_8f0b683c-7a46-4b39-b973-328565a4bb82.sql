-- ============================================
-- OPTIMIZATIONS FOR FRIEND SYSTEM PERFORMANCE
-- ============================================

-- 1. CREATE COMPOSITE INDEXES FOR FRIEND_REQUESTS
-- ============================================
CREATE INDEX IF NOT EXISTS idx_friend_requests_composite 
ON friend_requests(sender_id, receiver_id, status);

CREATE INDEX IF NOT EXISTS idx_friend_requests_status_created 
...
-- 2. CREATE COMPOSITE INDEXES FOR FOLLOWS
-- ============================================
...
-- 3. CREATE GIN INDEX FOR LOCATION SEARCH
-- ============================================
CREATE INDEX IF NOT EXISTS idx_profiles_location_gin 
ON profiles USING GIN (
  to_tsvector('simple', COALESCE(city, '') || ' ' || COALESCE(region, '') || ' ' || COALESCE(country, ''))
);


-- 4. CREATE MATERIALIZED VIEW FOR MUTUAL FRIENDS CACHE
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS mutual_friends_cache AS
SELECT 
  fr1.sender_id as user1_id,
  fr2.sender_id as user2_id,
  COUNT(DISTINCT fr1.receiver_id) as mutual_count
FROM friend_requests fr1
INNER JOIN friend_requests fr2 
  ON fr1.receiver_id = fr2.receiver_id
  AND fr1.sender_id < fr2.sender_id
WHERE fr1.status = 'accepted' 
  AND fr2.status = 'accepted'
GROUP BY fr1.sender_id, fr2.sender_id;

CREATE UNIQUE INDEX IF NOT EXISTS idx_mutual_friends_cache_users 
ON mutual_friends_cache(user1_id, user2_id);


-- 5. CREATE FUNCTION TO REFRESH MUTUAL FRIENDS CACHE
-- ============================================
...
-- 6. CREATE TRIGGER TO AUTO-REFRESH CACHE ON FRIEND ACCEPTANCE
-- ============================================
...
DROP TRIGGER IF EXISTS friend_request_accepted_refresh_cache ON friend_requests;
CREATE TRIGGER friend_request_accepted_refresh_cache
AFTER UPDATE ON friend_requests
FOR EACH ROW
EXECUTE FUNCTION trigger_refresh_mutual_friends();


-- 7. DROP OLD FUNCTION AND CREATE OPTIMIZED RPC: GET ONLINE FRIENDS (SINGLE QUERY)
-- ============================================
...
-- 8. OPTIMIZED RPC: GET BATCH FRIEND SUGGESTIONS
-- ============================================
...
BEGIN
  ...
  SELECT 
    ...
    p.region,
    p.country,
    COALESCE(mfd.mutual_count, 0)::INTEGER as mutual_friends_count,
...
-- 9. OPTIMIZED RPC: GET MUTUAL FRIENDS COUNT (USING CACHE)
-- ============================================
...
BEGIN
  ...
  SELECT mfc.mutual_count INTO mutual_count
  FROM mutual_friends_cache mfc
  ...
  IF mutual_count IS NULL THEN
    SELECT COUNT(DISTINCT fr1.receiver_id) INTO mutual_count
    FROM friend_requests fr1
    INNER JOIN friend_requests fr2 
      ON fr1.receiver_id = fr2.receiver_id
